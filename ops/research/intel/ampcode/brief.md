...
| **Subagent spawning** (parallel isolated execution) | **Counter** | Direct overlap with `aiki run`. Amp's subagents are ephemeral and model-optimized; Aiki's are persistent and workflow-tracked. Lean into durability and observability as the differentiator. ([ampcode.com/manual](https://ampcode.com/manual)) |
...
| **Handoff system** (context transfer) | **Copy (deprioritized)** | Automatic context analysis for task delegation could improve `aiki run` quality. Monitor and adapt when task delegation friction becomes a user complaint. ([ampcode.com/manual](https://ampcode.com/manual)) |
