...
| **Subagent orchestration with review** | Two-stage review at the subagent level: spec compliance, then code quality (`subagent-driven-development` skill) | `aiki task run` with full task context; `aiki review` for post-completion review | **Copy** — Adopt the two-stage review pattern (spec compliance + code quality) into `aiki review`. Aiki's task→diff provenance makes automated spec-compliance checking uniquely powerful. |
