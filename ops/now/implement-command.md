# Lanes, Sessions & Implement Template

**Date**: 2026-02-25
**Status**: Draft
**Purpose**: Introduce lanes and sessions as derived DAG concepts, add `task lane` query, and provide `aiki/implement` as the default orchestrator template.

**Dependencies**:
- [needs-context.md](needs-context.md) — `needs-context` link type and `--next-session` (replaces `--next-subtask`)
...
With `needs-context` links and `--next-session` in place (see [needs-context.md](needs-context.md)), we can build lane-based parallel orchestration on top.
...
3. **`aiki/implement` template** — default orchestrator that runs lanes in parallel, using `--next-session --lane` to drive each lane.
...
2. **`task lane`** — query primitive (this plan). Derives lanes from DAG, reports readiness.
3. **`aiki/implement` template** — default orchestrator (this plan). Gets ready lanes, drives each with `--next-session --lane`.
4. **`aiki build`** — convenience wrapper. `epic add` + create task from `aiki/implement` + run.
...
A lane is identified by its **first task ID** — the first task in the lane. This is a regular task ID, so prefix matching works the same way as everywhere else:

```bash
aiki task run <parent> --next-session --lane xtuttn --async
```

If the prefix is ambiguous (matches first tasks of multiple lanes), the error is lane-specific: `"Multiple lanes match prefix 'x', be more specific."`
...
```bash
# Show ready lanes with their sessions (default)
...
**`task lane <parent>` output (ready lanes):**
...
After all ready lanes are running:
...
**Readiness rules:**
- A lane is ready when all predecessor lanes have completed
...
# Lane ID = first task ID. Prefix matching supported.
...
`--lane` restricts `--next-session` to only consider subtasks within the named lane. The lane is identified by its first task's ID (prefix matching supported). This is how the orchestrator drives individual lanes.

### `--output last-id` for waiting on sessions

```bash
# Get the last task ID of the session for waiting
last_id=$(aiki task run <parent> --next-session --lane <lane-id> --async --output last-id)

# Wait for that session to complete
aiki task wait $last_id --any
```

When `--next-session` runs a multi-task session (needs-context chain), `--output last-id` returns the ID of the last task in that chain. Waiting for that task to complete means the entire session is done. For standalone tasks, it returns that task's ID.

This enables orchestrators to capture last task IDs and wait on them without needing to track all tasks in a session.

### `task wait --any-session`

```bash
# Wait for any running session under a parent task
aiki task wait <parent-id> --any-session
```

Waits for any running session (spawned via `task run --async`) under the parent task to complete. The CLI internally tracks which sessions are running. Returns error (non-zero exit) if no sessions are running.

This lets orchestrators wait on sessions without manually tracking task IDs across loop iterations.
...
while true; do
  ...
  # Start ready lanes (up to concurrency limit)
  for lane in $ready; do
    aiki task run {{data.target}} --next-session --lane $lane --async
  done
  
  # Wait for any session to finish (breaks when nothing running)
  aiki task wait {{data.target}} --any-session || break
done
```
...
2. Start sessions for ready lanes (CLI enforces concurrency limit internally)
3. Wait for any running session to finish
4. Loop back - finished session may have unblocked new lanes
5. Exit when `wait` fails (no sessions running and none can start)
...
4. Cross-lane dependency computation
5. Lane readiness: a lane is ready when all predecessor lanes have completed
6. `aiki task lane <parent>` — display ready lanes with their sessions (default)
...
2. Lane ID = first task ID of the lane. Prefix matching via existing ID resolver.
...
6. No match error: "No lane with first task matching '<id>' for task <parent>"

### Phase 3: `--output last-id` and `task wait --any-session`

1. Add `--output last-id` flag to `task run --async`
2. When running a needs-context chain, return the last task ID
3. For standalone tasks, return that task's ID
4. Add session tracking to CLI: track which tasks are running via `--async`
5. Add `--any-session` flag to `task wait <parent>`
6. Waits for any running session under parent to complete
7. Returns error if no sessions running
8. Reports which session (last task ID) completed

### Phase 4: Create `aiki/implement` template

1. Create `.aiki/templates/aiki/implement.md` with the orchestrator loop
2. Template is `type: orchestrator` for task graph semantics
3. Orchestrator loop: `task lane` → `--next-session --lane` → `task wait --any` → repeat

### Phase 5: Refactor `build.rs`

1. Replace `create_build_task()` with `create_implement_task()` using `aiki/implement` template
2. `build.rs` retains: plan validation, draft check, stale cleanup, epic find-or-create, blocker checks
3. Default template changes from `aiki/build` to `aiki/implement`
4. Keep `--template` flag for custom overrides
5. Remove `--loop` and `--lanes` flags from build (orchestration is template-driven)

### Phase 6: Tests

1. Unit tests: lane derivation — DAG decomposition, fan-out/fan-in, mixed sessions
2. Unit tests: lane readiness computation
3. Unit tests: `--lane` filter
4. Integration tests: `task lane` output format (default and `--all`)
5. Integration tests: orchestrator loop (implement template end-to-end)
...
| `--lane` with unknown ID | Error: "No lane with first task matching '<id>' for task <parent>" |
| `--lane` with ambiguous prefix | Error: "Multiple lanes match prefix '<prefix>', be more specific" |
| Session fails within lane | Lane cannot proceed to next session. Dependent lanes blocked. |
...
| `task lane` default | Shows ready lanes only | Matches `aiki task` default (show actionable). `--all` for full view. |
| Lane ID | First task ID | Stable (head doesn't change unless DAG restructured). No new ID system. Prefix matching reuses existing task ID resolver. |
| `--lane` filter | Scopes `--next-session` to lane by first task ID | Lets orchestrator manage per-lane execution. |
...
2. **Concepts** — Three: tasks, sessions, lanes. Sessions come from [needs-context.md](needs-context.md). Lanes are this plan's contribution.

3. **Persistence** — Lane derivation is deterministic from link structure. No persistence needed. `task lane` computes fresh each time.

4. **How does the orchestrator drive a lane?** — Calls `--next-session --lane <lane-id>` repeatedly (lane ID = first task ID, prefix matching supported). Each call runs one session and returns. When no more sessions remain, the lane is done.

5. **`task lane` default behavior** — Shows ready lanes only, matching `aiki task` convention. `--all` shows full decomposition with status.
