...
- [cli/src/commands/task.rs](../../cli/src/commands/task.rs) - `task run --next-session` and parent auto-start logic
...
- `task run --next-session --async --output id` returns exactly one 32-char lowercase ID
