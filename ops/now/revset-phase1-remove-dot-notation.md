# Revset Phase 1: Remove Dot-Notation Clauses

**Status:** Ready for implementation

## Problem

`build_task_revset_pattern` generates two substring matches per task ID — one for the task itself and one for `task={id}.` children. Dot-notation subtasks are no longer created; all subtasks use independent 32-char IDs linked via `subtask-of` edges. This doubles clause count for zero value.
...
| Single clause (no dot) | 0.24s | 0.37s | 0.32s | 0.54s |

Removing the dot-variant saves ~10-20%.
