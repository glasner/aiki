...
use aiki::flows::types::{
    Action, HookStatement, IfStatement, LogAction, OnFailure, OnFailureShortcut, ShellAction,
    SwitchStatement,
};
...
fn test_nested_if_in_switch() {
    ...
    cases.insert(
        "nested".to_string(),
        vec![HookStatement::If(IfStatement {
            condition: "1 == 1".to_string(),
            then: vec![HookStatement::Action(Action::Log(LogAction {
...
fn test_action_with_on_failure() {
    ...
    let statements = vec![HookStatement::Action(Action::Shell(ShellAction {
        shell: "false".to_string(), // Command that always fails
        timeout: None,
        on_failure: OnFailure::Statements(vec![HookStatement::Action(Action::Log(LogAction {
